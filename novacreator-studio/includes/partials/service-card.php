...
<div class="card-premium group relative reveal cursor-pointer touch-manipulation p-8 md:p-10 rounded-2xl transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl h-full flex flex-col">
    ...
    <div class="mt-auto pt-2">
        ...
        <?php endif; ?>
        <?php if ($cardLinkUrl): ?>
        <a href="<?php echo htmlspecialchars($cardLinkUrl); ?>" class="secondary-service-link inline-flex items-center gap-2 mt-4 text-sm sm:text-base font-medium transition-all duration-200 hover:opacity-80 min-h-[36px] touch-manipulation" style="color: var(--color-text-secondary);">
            <span><?php echo htmlspecialchars($cardLinkText); ?></span>
            <svg class="cta-arrow w-4 h-4 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5" aria-hidden="true">
