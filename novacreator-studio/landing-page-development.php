    ...
    <div class="container mx-auto px-4 md:px-6 lg:px-8">
        <div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="p-5 rounded-xl reveal" style="background-color: var(--color-bg); border: 1px solid var(--color-border);">
                ...
                <p class="font-semibold" style="color: var(--color-text);"><?php echo $currentLang === 'en' ? 'Paid traffic and one focused offer' : 'Платного трафика и одного целевого оффера'; ?></p>
            </div>
            <div class="p-5 rounded-xl reveal" style="background-color: var(--color-bg); border: 1px solid var(--color-border);">
                <p class="text-sm uppercase mb-2" style="color: var(--color-text-secondary);"><?php echo $currentLang === 'en' ? 'Launch window' : 'Срок запуска'; ?></p>
                <p class="font-semibold" style="color: var(--color-text);"><?php echo $currentLang === 'en' ? '10-14 business days' : '10-14 рабочих дней'; ?></p>
            </div>
            <a href="<?php echo getLocalizedUrl($currentLang, '/calculator?service=development'); ?>" class="p-5 rounded-xl reveal inline-flex items-center justify-between gap-3 transition-all hover:opacity-85" style="background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); color: #fff;">
                <span class="font-semibold"><?php echo $currentLang === 'en' ? 'Estimate landing cost' : 'Рассчитать стоимость лендинга'; ?></span>
